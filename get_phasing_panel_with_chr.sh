...
wget "http://ftp.1000genomes.ebi.ac.uk/vol1/ftp/data_collections/1000G_2504_high_coverage/working/20220422_3202_phased_SNV_INDEL_SV/1kGP_high_coverage_Illumina.chr${CHROM}.filtered.SNV_INDEL_SV_phased_panel.vcf.gz.tbi"

# Remove the 'chr' prefix from the chromosome names to match the Ensembl reference:
tabix -h 1kGP_high_coverage_Illumina.chr${CHROM}.filtered.SNV_INDEL_SV_phased_panel.vcf.gz chr${CHROM}:${START}-${END} | \
	bcftools view - -o 1kGP_high_coverage_Illumina.chr${CHROM}_${START}_${END}.filtered.SNV_INDEL_SV_phased_panel.bcf --output-type b
